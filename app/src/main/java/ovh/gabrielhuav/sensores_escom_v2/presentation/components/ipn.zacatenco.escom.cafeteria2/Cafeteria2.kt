...
class Cafeteria2 : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.escom_cafeteria2
            )
            ...
            mapView.post {
                // Configurar el mapa
                val normalizedMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_CAFETERIA2)
                mapView.setCurrentMap(normalizedMap, R.drawable.escom_cafeteria2)
    ...
    private fun returnToMainActivity() {
        ...
        val previousPosition = intent.getSerializableExtra("PREVIOUS_POSITION") as? Pair<Int, Int>
            ?: Pair(26, 35) // Posición por defecto si no hay previa
    ...
    private fun startBuildingActivity() {
        val intent = Intent(this, Cafeteria2::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            try {
                ...
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, position, "escom_cafeteria2")
                }
